...
studentas sukurimas() {
	...
	if (arrandom == 0) {
		...
		float* pazymiai = new float[nd];
		zmogus.vid = 0;
		for (int i = 0; i < nd; i++)
		{
			cout << "Iveskite " << i + 1 << "-aji pazymi: "; pazymiai[i] = pazymiu_imimas();
		...
		while (pasirinkimas == 1) {
			...
			float* tempi = new float[nd];

			for (int i = 0; i < nd - 1; i++) { tempi[i] = pazymiai[i]; };

			cout << "Iveskite " << nd << "-aji pazymi: "; tempi[nd-1] = pazymiu_imimas();

			pazymiai = tempi;
			zmogus.vid += pazymiai[nd - 1];
			delete[] tempi;
			cout << "ar ivesite papildoma pazymi?(0-ne, 1- taip)"; pasirinkimas = menu_pasirinkimas();
		}
